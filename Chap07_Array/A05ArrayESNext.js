...
// id와 매칭되는 인텍스 번호를 가져오자
x = objArray.findIndex( (item, index) => {
    // return item.id === 2;
    return item.name === 'HungBu';
    // if(item.id === 2) return index;
});

// 몇번째 있는지 인텍스 번호를 알았으니
console.log(x);

// 배열에서 인텍스 번호를 참조하면 객체 전체를 얻을 수 있다.
